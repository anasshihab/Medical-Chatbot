...
from app.tools.web_reader import WebPageReaderTool
...
class MedicalChatAgent:
    ...
    async def process_message(
        ...
        # 2. Decide action
        decision = await self.decision_maker.decide_action(user_message, conversation_history)
        ...
        if decision["action"] == "keyword_search":
        ...
        elif decision["action"] == "read_url":
            # Execute URL reader
            url = decision["params"].get("url", "")
            if not url and "http" in user_message:
                # Extract URL from message if not in params
                import re
                urls = re.findall(r'(https?://\S+)', user_message)
                if urls:
                    url = urls[0]
            
            tool_result = await self.web_reader.execute(url=url)
            ...
            # Pass through to main generation
            pass
